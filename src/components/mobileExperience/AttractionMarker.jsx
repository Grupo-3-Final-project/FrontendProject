function AttractionMarker({ attraction, onSelect }) {
  ...
  return (
    <button
      type="button"
      className={`absolute ${positionClass} cursor-pointer border-0 bg-transparent p-0 text-left`}
      onClick={() => onSelect(attraction)}
      aria-label={`Ver informacion de ${name}`}
    >
      <span className="absolute left-1/2 top-1/2 z-10 w-max -translate-x-1/2 -translate-y-[120%]">
